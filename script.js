...
async function getDataFromUrl(URL) {
  try {
    ...
    if (!response.ok) throw new Error("відповідь не є вдалою");
...
let gathData = gatherData();

async function displayData() {
  console.log((await gathData.next()).value);
  console.log((await gathData.next()).value);
  console.log((await gathData.next()).value);
}
// Розкоментуйте після виконання завданння
...
async function displayCount() {
  let nextVal = await countdown.next();
  while (nextVal.done === false) {
    console.log(nextVal.value);
    nextVal = await countdown.next();
  }
}
